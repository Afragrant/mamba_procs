...
* the normalized position grid ``pos`` is appended as an extra channel so the
  sequence model receives the spatial phase it needs to reconstruct a
  spatially-varying response from otherwise-constant parameters,
* targets are standardized per channel.

Input tensor shape: ``(N, seq_len, 7)``  (6 features + position).
...
class PCData:
    ...
    def _build_inputs(self, X: np.ndarray) -> np.ndarray:
        ...
        posb = np.broadcast_to(self.pos[None, :, None], (n, seq, 1))
        return np.concatenate([xb, posb], axis=2).astype(np.float32)
