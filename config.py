...
# KEQ 跨 4 个数量级 (1e4~1e8) -> 对数采样.  EI 仅 2.67 倍, 线性即可, 不在此列.
LOG_SAMPLE_VARS = {'KEQ'}

# 跨数量级的变量同样须在对数空间做归一化: 否则线性 Min-Max 会把低端 ~3 个数量级
# 的样本全挤到 x_norm≈-1 的一小段, 网络无法区分 (与对数采样的均衡覆盖自相矛盾).
# 对这些变量, data_utils 先取 log10 再做 Min-Max, 使 log10(x) 均匀映射到 [-1,1].
# 必须与 LOG_SAMPLE_VARS 配对: 跨数量级 -> 既对数采样又对数归一化.
LOG_NORM_VARS = {'KEQ'}
