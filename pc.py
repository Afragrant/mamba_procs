...
def pantograph_params(ptype: int, v_kmh: float):
    ...
    table = {
        1: (7.12, 6.00, 5.80, 9430, 14100, 0.1, 0, 0, 70, 100 + f_aero),
        2: (7.51, 5.855, 4.645, 8380, 6200, 80, 0, 0, 70, 100 + f_aero),
        3: (5, 9.978, 9, 6000, 8971, 0.5, 5, 5, 350, 100 + f_aero),
        4: (11.4, 7.11, 8.03, 21111.72, 233.01, 13.68, 24.58, 90.35, 5678, 100 + f_aero),
    }
...
def compute_busbar_positions(LS: float, x_j: np.ndarray, l_mz: float, tol_sup: float = TOL_SUP) -> np.ndarray:
    """Busbar joint positions every l_mz, auto-shifted away from supports."""
...
def newmark_step(M, C, K, F, Y_k, V_k, A_k, dt, beta=0.25, gamma=0.5):
    """Single Newmark-beta step."""
    a0 = 1.0 / (beta * dt**2)
    a1 = gamma / (beta * dt)
    a2 = 1.0 / (beta * dt)
    a3 = 1.0 / (2.0 * beta) - 1.0
    ...
    Kt = K + a0 * M + a1 * C
    Ft = F + M @ (a0 * Y_k + a2 * V_k + a3 * A_k) + C @ (a1 * Y_k + a4 * V_k + a5 * A_k)

    Y_k1 = solve(Kt, Ft, assume_a='pos')
    A_k1 = a0 * (Y_k1 - Y_k) - a2 * V_k - a3 * A_k
    V_k1 = V_k + a6 * A_k + a7 * A_k1
    return Y_k1, V_k1, A_k1
...
def run_simulation(
    ...
    # Support positions (interior, between spans)
    ...
    # Busbar joint positions and modal mass contribution
    ...
    # Gravity loads (beam + supports + busbar joints)
